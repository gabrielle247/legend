...
class BillingEngine {
  ...
  BillingEngine._internal();

  final _uuid = const Uuid();
  final _financeRepo = PowerSyncFinanceRepository();
  ...
  Future<void> _runForSchool(String schoolId, DateTime now) async {
    final term = await _getActiveTerm(schoolId);
    final year = await _getActiveYear(schoolId);
    final students = await _loadActiveStudents(schoolId);
  ...
  Future<void> _runMonthlyFixed(_StudentBillingRow row, Term term, DateTime now) async {
  ...
  Future<void> _runMonthlyCustom(_StudentBillingRow row, DateTime now) async {
  ...
  Future<void> _runTermly(_StudentBillingRow row, Term term, DateTime now) async {
  ...
  Future<void> _runYearly(_StudentBillingRow row, _AcademicYear year, DateTime now) async {
  ...
  Future<void> _ensureInvoice(
    _StudentBillingRow row,
    DateTime dueDate,
    String title, {
    String? termId,
    required String periodLabel,
  }) async {
    final invoiceId = await _findInvoiceId(row.schoolId, row.studentId, title);
    final itemDescription = "Tuition - $periodLabel";

    if (invoiceId != null) {
      final hasItem = await _invoiceHasItem(invoiceId, itemDescription);
      if (!hasItem) {
        ...
        await _financeRepo.addInvoiceItem(invoiceId, item);
      }
      return;
    }
    ...
    await _financeRepo.createInvoice(inv, [item]);
  }
  ...
  Future<Term?> _getActiveTerm(String schoolId) async {
  ...
  Future<_AcademicYear?> _getActiveYear(String schoolId) async {
    final row = await db.getOptional(
  ...
  Future<List<_StudentBillingRow>> _loadActiveStudents(String schoolId) async {
  ...
  Future<String?> _findInvoiceId(String schoolId, String studentId, String title) async {
    final row = await db.getOptional(
  ...
  Future<bool> _invoiceHasItem(String invoiceId, String description) async {
    final row = await db.getOptional(
